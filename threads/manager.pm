...
sub UNIVERSAL::SharedHash : ATTR(ANY)
{
   ...
   $ev .= q¬
    ...
    ¬;
   $ev .= 'while($'.$_[0].'::lock_1_data){} $'.$_[0].'::lock_1_data++;';
   $ev .= q¬
         foreach (keys %{$data})
         {
   ...
   ## metodo clear
   $ev .= '*'.$_[0].'::clear = sub { my $self = shift; delete $self->{$_} foreach (keys %{$self}); 1 };';  
...
sub new
{
   ...
   {
      require Carp;
      Carp::croak('USE: '.$/.'my $max_threads = 10; '.$/.__PACKAGE__.'->new($max_threads)');
   }
   ...
   if (ref $arg eq 'HASH' and exists $arg->{'timeout'})
   {
      ...
      unless(looks_like_number($timeout__) and int($timeout__) == $timeout__)
      {
         require Carp;
         Carp::croak('USE: '.$/.__PACKAGE__.'->new($max_threads, {timeout => 60})');         
      }
   }
...
sub create
{
   ...
   while (1)
   {
      ...
      if (scalar(threads->list(threads::running)) < $self->{'max'})
      {
         ...
         $tids__{$th->tid} =  ($tout || $timeout__) ?  time + ($tout || $timeout__) : 0;
         return $th->tid;
      }
      sleep 1;
   }
...
##              }
##
sub start_semaphore { while ($lock__){select(undef, undef, undef, 0.5) } $lock__++; }
...
sub __detach 
{
   ...
   {
      $_->detach and $_->kill('KILL') and $_->exit and delete $tids__{$_->tid} 
   }
   1;
}
